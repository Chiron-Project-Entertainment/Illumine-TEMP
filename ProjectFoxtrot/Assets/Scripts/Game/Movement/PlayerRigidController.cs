...
public class PlayerRigidController : MonoBehaviour
{
    ...
    /// Collision checks
    /// </summary>
    private struct BoxCheck
    {
        public Vector3 position;
        public Vector3 size;
        public BoxCheck(Vector3 position, float size) { this.position = position; this.size = Vector3.one * size; }
    }
    private BoxCheck groundCheck { get { return new BoxCheck(transform.TransformPoint(boxCollider.center) - transform.up * (boxCollider.size.y / 2), 0.4f); } }
    private BoxCheck ceilingCheck { get { return new BoxCheck(transform.TransformPoint(boxCollider.center) + transform.up * crouchHeight, 0.4f); } }
    ...
    private void Awake()
    {
        ...
        boxCollider.size = new Vector3(boxCollider.size.x, standUpHeight, boxCollider.size.z);
    ...
    private void Update()
    {
        ...
        OnGround = Physics.CheckBox(groundCheck.position, groundCheck.size / 2, transform.rotation, groundMask, QueryTriggerInteraction.Ignore);
        ceilingAbove = Physics.CheckBox(ceilingCheck.position, ceilingCheck.size / 2, transform.rotation, groundMask, QueryTriggerInteraction.Ignore);
        #endregion
        ...
        if (IsWallRunning)
        {
            transform.position = transform.position + transform.TransformDirection(wallRunningDirection) * Time.deltaTime;
        ...
        {
            boxCollider.size = new Vector3(boxCollider.size.x, crouchHeight, boxCollider.size.z);
            IsCrouching = true;
            if (OnGround && !IsSprinting)
            {
                transform.position = transform.position + transform.up * crouchHeight * -1;
            }
        }
        ...
        rb.velocity = velocity.x * transform.right + velocity.y * transform.up + velocity.z * transform.forward;

        // Move the player based on the velocity.
        //transform.position = transform.position + (velocity.x * transform.right + velocity.y * transform.up + velocity.z * transform.forward) * Time.deltaTime;
        #endregion
    ...
    {
        Gizmos.DrawCube(groundCheck.position, groundCheck.size);
        Gizmos.DrawCube(ceilingCheck.position, ceilingCheck.size);
    }
    #endregion
}
